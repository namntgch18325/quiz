<template>
  <div ref="test" class="app-container">
    <div class="filter-container">
      <el-button type="success" @click="handleCreate()">{{ $t('button.create') }}</el-button>
      <el-button type="danger" @click="handleDeleteManyQuiz()">{{ $t('button.delete') }}</el-button>
    ...
    >
        ...
        <template slot-scope="scope">
          ...
          >
            <el-table :data="scope.row.options">
              <el-table-column :label="$t('button.options')" max-width="400" property="option" />
            </el-table>
      ...
      >
        <template slot-scope="{row}">
          <el-button @click="handleShow(row)">{{ $t('button.show') }}</el-button>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="{row}">
          ...
          <el-button type="danger" icon="el-icon-delete" circle @click="handleDeleteOneQuiz(row)" />
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-row>
          <el-col :span="11">
            <input id="pic_ja" type="file" accept="image/*" :disabled="tempAction.hidden" @change="preView_ja">
            <div id="preview">
              <center>
                <el-image
          ...
          <el-col :span="2"><div class="space" /></el-col>
          <el-col :span="11">
            <input id="pic_en" type="file" accept="image/*" :disabled="tempAction.hidden" @change="preView_en">
            <div id="preview">
              <center>
                <el-image
                  style="width: 200px; height: 200px; margin-top: 10px; margin-botton: 10px;"
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-input v-model="scope.row.option_ja" :placeholder="$t('popup.quiz.pEnterJapanese')" :readonly="tempAction.hidden" @change="textChange(scope.$index)" />
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              <el-input v-model="scope.row.option_en" :placeholder="$t('popup.quiz.pEnterEnglish')" :readonly="tempAction.hidden" @change="textChange(scope.$index)" />
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              <el-checkbox v-model="scope.row.correct" :disabled="tempAction.hidden" @change="handletableCorrect(scope.row)" />
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              <el-button type="danger" icon="el-icon-delete" circle @click="handleDeleteOption(scope.$index)" />
            </template>
          </el-table-column>
        ...
        <center v-if="tempAction.hidden == false">
          <el-row>
            <el-button type="danger" class="btn-insert" icon="el-icon-plus" circle @click="insertRow()" />
          </el-row>
        </center>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-if="tempAction.hidden == false" type="primary" :disabled="isproces" @click="submit()">
          {{ $t('button.save') }}
        </el-button>
      </div>
...
import { fetchList, fetchData, addQuiz, deleteQuiz, editQuiz } from '@/api/quiz'; // import file call api
...
export default {
  data() {
    return {
      ...
      ruleForm: { // obj store data in popup create/edit/sho
        title_ja: '',
        title_en: '',
        sentence_ja: '',
        sentence_en: '',
      ...
      img_en: '',
      img_ja: '',
      rules: { // rules for validate form in popup
        title_ja: [
          { required: true, message: this.$t('popup.quiz.validate.rtitle'), trigger: 'blur' },
          { max: 255, message: this.$t('popup.quiz.validate.rtitle'), trigger: 'blur' },
        ],
        title_en: [
          { required: true, message: this.$t('popup.quiz.validate.rtitle'), trigger: 'blur' },
          { max: 255, message: this.$t('popup.quiz.validate.rtitle'), trigger: 'blur' },
        ...
        sentence_ja: [
          { required: true, message: this.$t('popup.quiz.validate.rsentence'), trigger: 'blur' },
          { max: 255, message: this.$t('popup.quiz.validate.lsentence'), trigger: 'blur' },
        ],
        sentence_en: [
          { required: true, message: this.$t('popup.quiz.validate.rsentence'), trigger: 'blur' },
          { max: 255, message: this.$t('popup.quiz.validate.lsentence'), trigger: 'blur' },
        ...
        explain_correct_ja: [
          { required: true, message: this.$t('popup.quiz.validate.rexCorrect'), trigger: 'blur' },
          { max: 255, message: this.$t('popup.quiz.validate.lexCorrect'), trigger: 'blur' },
        ],
        explain_correct_en: [
          { required: true, message: this.$t('popup.quiz.validate.rexCorrect'), trigger: 'blur' },
          { max: 255, message: this.$t('popup.quiz.validate.lexCorrect'), trigger: 'blur' },
        ...
        explain_incorrect_ja: [
          { required: true, message: this.$t('popup.quiz.validate.rexIncorrect'), trigger: 'blur' },
          { max: 255, message: this.$t('popup.quiz.validate.lexIncorrect'), trigger: 'blur' },
        ],
        explain_incorrect_en: [
          { required: true, message: this.$t('popup.quiz.validate.rexIncorrect'), trigger: 'blur' },
          { max: 255, message: this.$t('popup.quiz.validate.lexIncorrect'), trigger: 'blur' },
      ...
      sortable: null,
      tempAction: { // Store action and disabel textbox when show
        Action: '',
        hidden: false,
      },
  ...
  computed: { // refeash page when change languages
    i18nLang() {
  ...
  methods: {
    showfull() { // methods change title.icon when create/edit/show
    ...
    toggleSelection(rows) { // Choose the right question
      if (rows) {
        rows.forEach(row => {
    ...
    handleSelectionChange(val) { // methods when click btn create
      this.multipleSelection = [];
      for (var i = 0; i < val.length; i++) {
        this.multipleSelection.push(val[i].id);
      }
    },
    handleCreate() { // Event click Create
    ...
    submit() { // method when click btn save in popup
      this.$refs['ruleform'].validate((valid) => { // for check validate
        if (valid) {
          if (this.tempAction.Action === 'Create') { // check create
            this.ruleForm.picture_ja = document.getElementById('pic_ja').files[0];
            this.ruleForm.picture_en = document.getElementById('pic_en').files[0];
            var checkOption = false;
            for (var x = 0; x < this.ruleForm.options.length; x++) {
              if (this.ruleForm.options[x].option_ja === '' || this.ruleForm.options[x].option_en === '') {
                checkOption = true;
                break;
              }
            }
            if (checkOption === false && this.ruleForm.options.length >= 1) {
              ...
              if (count !== this.ruleForm.options.length) {
                ...
                if (this.ruleForm.picture_ja !== undefined) {
                  file.append('picture_ja', this.ruleForm.picture_ja);
                }
                if (this.ruleForm.picture_en !== undefined) {
                ...
                file.append('explain_correct_ja', this.ruleForm.explain_correct_ja);
                file.append('explain_correct_en', this.ruleForm.explain_correct_en);
                file.append('explain_incorrect_ja', this.ruleForm.explain_incorrect_ja);
                file.append('explain_incorrect_en', this.ruleForm.explain_incorrect_en);
                for (var j = 0; j < options.length; j++) {
                  file.append('options' + '[' + j + '][option_ja]', options[j].option_ja);
                  file.append('options' + '[' + j + '][option_en]', options[j].option_en);
                  file.append('options' + '[' + j + '][correct]', options[j].correct);
                  file.append('options' + '[' + j + '][position]', options[j].position);
                }
                file.append('languages', this.$root.$i18n.locale);
                var resCreate = addQuiz(
                  file
                );
                resCreate.then((data) => {
          ...
          if (this.tempAction.Action === 'Edit') { // check Edit
            var checkOptionEdit = false;
            for (var c = 0; c < this.ruleForm.options.length; c++) {
              if (this.ruleForm.options[c].option_ja === '' || this.ruleForm.options[c].option_en === '') {
                checkOptionEdit = true;
                break;
              }
            }
            if (checkOptionEdit === false && this.ruleForm.options.length >= 1) {
              var countEdit = 0;
              ...
              if (countEdit !== this.ruleForm.options.length) {
                ...
                this.ruleForm.picture_ja = document.getElementById('pic_ja').files[0];
                this.ruleForm.picture_en = document.getElementById('pic_en').files[0];
                if (document.getElementById('pic_ja').files.length === 1){
                  fileEdit.append('picture_ja', this.ruleForm.picture_ja);
                }
                if (document.getElementById('pic_en').files.length === 1){
                  fileEdit.append('picture_en', this.ruleForm.picture_en);
                }
                fileEdit.append('explain_correct_ja', this.ruleForm.explain_correct_ja);
                fileEdit.append('explain_correct_en', this.ruleForm.explain_correct_en);
                fileEdit.append('explain_incorrect_ja', this.ruleForm.explain_incorrect_ja);
                fileEdit.append('explain_incorrect_en', this.ruleForm.explain_incorrect_en);
                for (var u = 0; u < optionsEdit.length; u++) {
                  fileEdit.append('options' + '[' + u + '][option_ja]', optionsEdit[u].option_ja);
                  fileEdit.append('options' + '[' + u + '][option_en]', optionsEdit[u].option_en);
                  fileEdit.append('options' + '[' + u + '][correct]', optionsEdit[u].correct);
                  fileEdit.append('options' + '[' + u + '][position]', optionsEdit[u].position);
                  fileEdit.append('options' + '[' + u + '][status]', optionsEdit[u].status);
                  if (optionsEdit[u].status !== -2) {
                    fileEdit.append('options' + '[' + u + '][id]', optionsEdit[u].id);
                  }
                }
                fileEdit.append('languages', this.$root.$i18n.locale);
                fileEdit.append('current_page', this.listQuery.page);
                var resEditQuiz = editQuiz(
                  fileEdit
                );
    ...
    insertRow() { // Insert Row for input Options
    ...
    async DeleteOneQuiz(row) {
      ...
      const resDeleteOne = await deleteQuiz({
        id_list: [Quiz.id],
        current_page: current_page,
        languages: this.$root.$i18n.locale,
      });
      if (resDeleteOne.message === 'delete successfully') {
        this.$message({
          message: this.$t('popup.quiz.deleteSuccess'),
          type: 'success',
        });
        this.listLoading = true;
        this.ListQuiz = resDeleteOne.infor_page.data;
        this.total = resDeleteOne.infor_page.total;
        this.listQuery.last_page = resDeleteOne.infor_page.last_page;
        this.listQuery.page = resDeleteOne.infor_page.current_page;
        this.lengthData = resDeleteOne.infor_page.data.length;
        this.listLoading = false;
        ...
        this.dialogVisibleDelete = false;
        this.$message({
          message: this.$t('popup.quiz.deleteFailed'),
          type: 'warning',
        });
      }
    },
    async DeleteManyQuiz() {
      ...
      if (this.multipleSelection.length === 0){
        ...
        const resDeleteMany = await deleteQuiz({
          id_list: dataDelete,
          current_page: current_page,
          languages: this.$root.$i18n.locale,
        });
        ...
        if (resDeleteMany.message === 'delete successfully') {
          ...
          this.listLoading = true;
          this.ListQuiz = resDeleteMany.infor_page.data;
          this.total = resDeleteMany.infor_page.total;
          this.last_page = resDeleteMany.infor_page.last_page;
          this.listQuery.page = resDeleteMany.infor_page.current_page;
          this.lengthData = resDeleteMany.infor_page.data.length;
          this.listQuery.keyword = '';
          this.listLoading = false;
          this.dialogVisibleDelete = false;
        } else {
    ...
    processEditQuiz(){
      ...
      for (var x = 0; x < this.tempPosition.length; x++) {
        if (this.ruleForm.options[x].status !== -2 && this.ruleForm.options[x].position !== this.tempPosition[x]) {
