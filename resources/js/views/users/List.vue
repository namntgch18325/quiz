<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" :label="$t('profile.role')" width="120">
        <template slot-scope="scope">
          <span>{{ scope.row.roles.join(', ') }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('profile.edit')" width="100">
        <template slot-scope="{row}">
          <el-button v-permission="['manage user']" type="primary" size="small" icon="el-icon-edit" @click="openPop(row)">
            {{ $t('profile.edit') }}
          </el-button>
        </template>
      </el-table-column>
      <el-table-column align="center" :label="$t('profile.delete')" width="100">
        <template slot-scope="scope">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button :disabled="isproces" type="primary" @click="confrimEdit()">{{ $t('profile.confirm') }}</el-button>
      </span>
    </el-dialog>
    <!-- pop-up Create new user  -->
    <el-dialog width="400px" :title="$t('list.popCreateMew')" :visible.sync="dialogFormVisible">
      <div v-loading="userCreating" class="form-container">
        <el-form ref="userForm" :rules="rules" :model="newUser" label-position="left" label-width="150px" style="max-width: 500px;">
          <el-form-item :label="$t('user.role')" prop="role">
            <el-select v-model="newUser.role" class="filter-item" :placeholder="$t('list.placeHolderRole')">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        role: [{ required: true, message: this.$t('list.messRoleRequired'), trigger: 'change' }],
        name: [{ required: true, message: this.$t('list.messNameRequired'), trigger: 'blur' }],
        email: [
          { required: true, message: this.$t('list.messEmailRequired'), trigger: 'blur' },
          { type: 'email', message: this.$t('list.messInputCorrectE'), trigger: ['blur', 'change'] },
        ],
        password: [{ required: true, message: this.$t('list.messPasswordRequired'), trigger: 'blur' }],
  ...
  methods: {
    ...
    openPop(row){ // open pop edit user
      ...
      this.$refs['userForm'].validate((valid) => {
        if (valid) {
          ...
          userResource
            ...
            .then(response => {
              this.$message({
                message: this.$t('list.messCfNewUserFont') + this.newUser.name + '(' + this.newUser.email + this.$t('list.messCfNewUserTail'),
                type: 'success',
                duration: 5 * 1000,
              });
    ...
    confrimEdit(){ // submit when click Confrim
